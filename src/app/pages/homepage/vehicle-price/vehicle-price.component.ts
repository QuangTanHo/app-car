...
public parseOptionData(response: any) {
  ...
  this.dataDongXe = Array.from(options).map(option => ({
    id: option.getAttribute('value') !== null ? parseInt(option.getAttribute('value')!) : null,
...
public parseOptionDataVehi(response: any) {
  ...
  this.dataPhienBan = Array.from(options).map(option => ({
    id: parseInt(option.getAttribute('value') ?? '0', 10), // Default to '0' if value is null
    name: option.textContent?.trim() ?? '' // Default to empty string if textContent is null
  }));
}
